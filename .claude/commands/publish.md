...
- Read the `**Platforms:**` line under `### 3.1 BA` in `project/project_config.md` — this is only the project-wide default (set once via `/config`). A BA can override it per feature, but that override is written into BA's own local `env_<slug>.md` on the `dev/BA` branch, not back into `project_config.md` — so this line is never guaranteed to reflect what was actually decided for a specific feature, only the fallback when nothing more specific is known. QA's own `env_<slug>.md` (the `### 3.2 QA` template) doesn't carry a per-feature Platforms field either, so this project-level default is the baseline source. It's a comma-separated subset of `BE`, `FE`, `Mobile`, `Auto Test` — `Auto Test` is included in this same field even though it isn't a deployment platform, since it's simpler to set it alongside BE/FE/Mobile than as a separate switch.
- Also list the child pages that already exist under the "AI Doc folder" page (from `env_<slug>.md`) and note which of `AI Doc for BE - <Feature Name>`, `AI Doc for FE - <Feature Name>`, `AI Doc for Mobile - <Feature Name>`, `AI Doc for Auto Test - <Feature Name>` are already present — an existing page is stronger evidence than the project default, since it reflects what was actually decided for this specific feature (the BA side may have overridden the project default per-feature, per the note above). Treat any role with an existing page as applicable even if it isn't in the project default. On a feature's very first publish, no such page exists yet, so the project default is all that's available — if that turns out to miss a role the BA actually intended, it can be corrected by publishing again after the missing role's page is manually reconciled.
- The applicable roles are the union of both checks above.
