...
4. Read `workspace/<folder-name>/input/env_<slug>.md` once — if missing, stop: "Environment file not found. Run `/start <Feature Name>` first." Keep its content for every check below and for Step 1 (Confluence output page link) and Step 2 (Platforms line, AI Doc folder page) so none of them need to re-read this file:
   - Scan it for unfilled placeholders (pattern `<...>`):
     - If any found → stop and inform the user:
       ...
       ```
   - Check `workspace/<folder-name>/qa_doc_<slug>.md` exists — if missing, stop: "QA Doc not found. Run `/package <Feature Name>` or `/review <Feature Name>` first."
...
Publish the contents of `workspace/<folder-name>/qa_doc_<slug>.md` to the Confluence page listed under "Confluence output pages: QA Doc" in `env_<slug>.md` (already read in Pre-flight). This always runs — it is not configurable via Task Automation.
...
Use the `### 4.2 QA` action entries already parsed in Pre-flight — do not re-read `project/project_config.md`.
