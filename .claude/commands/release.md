...
1. In `<dev-branch>`'s own `README.md`, update the version line right under the title (e.g. `v1.0`) to the confirmed version: just `v<version>`, no parenthetical — this applies to all three targets, regardless of any Requires answer collected in Step 3.4/4.5 (that answer is only for main's table in Step 8.2, never repeated here).
