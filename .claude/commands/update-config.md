...
These are the exact same 23 questions `/config` asks, grouped by section — reuse `.claude/commands/config.md`'s own phrasing/format/update-rules for whichever one the user picks (its numbered sub-questions 1.a-d, 2.a-n, 3.a, and 4.a-d), rather than duplicating that text here.

- **Project Setup**: 1. Project Name — 2. MCP Config: Atlassian — 3. MCP Config: Figma — 4. Language
- **Context Sync**: 5. Context — 6. Sample Doc — 7. Business Rules: Principles — 8. Business Rules: Shared References — 9. Data Definition: Shared References — 10. UI Behavior: Principles — 11. UI Behavior: Shared References — 12. Navigation — 13. Flow — 14. Messages — 15. Test Scenarios: Principles — 16. Test Scenarios: Shared References — 17. Test Cases: Principles — 18. Test Cases: Shared References
- **Task Environment**: 19. Platforms
- **Task Automation**: 20. Jira actions (BA) — 21. Confluence actions (BA) — 22. Jira actions (QA) — 23. Confluence actions (QA)

## Steps

1. Read `project/project_config.md` and show the user all 23 items above with their **current value** next to each — as a plain numbered list, not a select UI (23 options doesn't fit a select box anyway). Several items can hold more than one entry (Atlassian once multiple sites are set up via `/connect-local-mcp`, and every Context Sync category 5-18 since the user can paste a list of `<name>: <url>` pairs) — for those, show a count and the entry names rather than assuming a single value, e.g. `5. Context — 2 entries (BRD, roadmap)`. Use `(not set)` for anything still a placeholder or empty. Example shape:
   ```
...
2. Once the user names one (by number or name), update **only that item**:
   ...
   - **Any multi-entry item (5-18) that already has entries** — the user's answer to the question above tells you what they want (e.g. "add another one", "replace the roadmap link", "remove BRD"); apply it against the existing list rather than assuming a wholesale replacement.
