...
| 3 | Generate & Package | `/gen-doc` (gen-test-scenarios → gen-test-cases → package, run automatically in sequence) | `qa_doc_<slug>.md` |
...
- **Step 3 — Generate & Package**: ✓ Ready if `qa_doc_<slug>.md` exists. ✗ Missing otherwise. Do not check this step if Step 2 is not ✓ Ready. Do not inspect the 2 intermediate docs individually — `qa_doc_<slug>.md` existing is proof the whole sequence completed, since `/package` itself requires both of them to exist first.
