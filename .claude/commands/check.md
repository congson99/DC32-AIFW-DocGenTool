...
| 3 | Generate & Package | `/gen-doc` (resolve-assumptions → gen-test-scenarios → gen-test-cases → review, run automatically in sequence — review creates/refreshes `qa_doc_<slug>.md` itself, so `/package` isn't a separate sub-step here) | `qa_doc_<slug>.md`, plus the `**Review:**` line in `input/env_<slug>.md` |
