...
| 3 | Generate & Package | `/gen-doc` (brief → dependencies → ac → business_rule → data_definition → navigation → flow → ui_behavior → messages → review, run automatically in sequence — review creates/refreshes `ba_doc_<slug>.md` itself, so `/package` isn't a separate sub-step here) | `ba_doc_<slug>.md`, plus the `**Review:**` line in `input/env_<slug>.md` |
...
- **Step 3 — Generate & Package**: ✓ Ready if `ba_doc_<slug>.md` exists AND `env_<slug>.md` contains a `**Review:** ✓ Completed` line. ⚠ "Packaged but not yet reviewed — run /review" if `ba_doc_<slug>.md` exists but that line is missing. ✗ Missing if `ba_doc_<slug>.md` doesn't exist. Do not check this step if Step 2 is not ✓ Ready. Do not inspect the 9 intermediate docs individually — `ba_doc_<slug>.md` existing is proof `/gen-messages` (which `/package` or `/review` both require) already produced all 9 of them.
