...
2. List what will happen:
   ...
   - All feature folders under `workspace/` will be deleted, if any exist (e.g. left over from switching to a BA/QA doc-gen tool branch in this same clone — `workspace/` is gitignored and persists across branch switches).
...
5. Delete all contents inside `workspace/` (all feature subfolders and files), if any exist — this confirmation already covers it, no separate confirmation needed.
